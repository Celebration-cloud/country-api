...
export default function ActionAreaCard({ output, filter, loading, turn, search }) {
  ...
  const full = search.map((item) => item.name.toLowerCase())
  console.table(full)
    return (
      <>
        {loading ? (
          <h4 style={turn === false ? { color: "black" } : { color: "white" }}>
            Loading...
          </h4>
        ) : (
          <div className="resultPage">
            ...
            {/* NOT FOUND */}
            {!full.includes(output.toLowerCase()) && (
              <>
                <h4
                  style={
                    turn
                      ? { color: "black", textAlign: "center", height: "100%" }
                      : { color: "white", textAlign: "center", height: "100%" }
                  }
                >
                  Not Found
                </h4>
